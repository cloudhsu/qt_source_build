...
echo "Script for build QT SDK ${QT_FULL_VERSION}."
...
BUILD_FOLDER="build-qt-${OSTYPE}-${MTYPE}-${QT_FULL_VERSION}"
...
prefix_path=$PWD/qt-${OSTYPE}-${MTYPE}-${QT_FULL_VERSION}

cd ${BUILD_FOLDER}

../qt-everywhere-src-${QT_FULL_VERSION}/configure \
      -opensource -confirm-license -release -strip -rpath \
      ...
      -skip qttranslations -skip qtsensors -skip qtserialbus -skip qtserialport
...
cd ..
# using tar.xz to tar
tar Jcvf qt-${OSTYPE}-${MTYPE}-${QT_FULL_VERSION}.tar.xz qt-${OSTYPE}-${MTYPE}-${QT_FULL_VERSION}
